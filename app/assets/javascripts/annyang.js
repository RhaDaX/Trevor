...
if (annyang) {
      ...
      'Trevor éteins la lumière' : function() {
        $.ajax("/bulb/switch_off")
      }, 
      'Trevor allume la lumière' : function() {
        $.ajax("/bulb/switch_on")
      },
     
  		'Trevor freebox *term' : function(term) {
  			term = term.replace( /\s/g, "")
  			term = term.toLowerCase()
  			$.ajax("/pages/httprequest?parameters="+term)
        $.ajax("/pages/speak?text=ok")
  		}, 
      // Fonction pour test reconnaisance vocale.
      '*term' : function(term) {
        console.log(term);
      },
