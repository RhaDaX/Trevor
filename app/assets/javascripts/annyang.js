...
//window.onload = function(){
  if (annyang) {
    ...
    /*var commands = {
            't f 1': function() {
                $('#hello').show();
                $.ajax("/pages/httprequest?parameters=1")
            },
            'france deux': function() {
            	$.ajax("/pages/httprequest?parameters=2")
            },
            'france 3': function() {
                $('#hello').show();
                $.ajax("/pages/httprequest?parameters=3")
            },
            'canale plusse': function() {
                $('#hello').show();
                $.ajax("/pages/httprequest?parameters=4")
            },
            'arté': function() {
                $('#hello').show();
                $.ajax("/pages/httprequest?parameters=5")
            },
            'm six': function() {
                $('#hello').show();
                $.ajax("/pages/httprequest?parameters=6")
            },
    }; */
